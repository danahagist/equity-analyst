...
def _packet(rows, last_price=100.0, ticker="TEST"):
    return {"ticker": ticker, "as_of": "2026-07-08", "last_price": last_price,
            "forecast_rows": rows}


def _row(label, point, lower, upper, beats):
    return {"label": label, "point": point, "lower": lower, "upper": upper,
            "beats_baseline": beats, "target_date": "2026-08-06", "model": "X",
            "interval_level": 80, "n_windows": 16}


def test_plan_uses_near_band_and_long_target() -> None:
    plan = plan_from_packet(_packet([
...
def test_levels_report_carries_disclaimer() -> None:
    plan = plan_from_packet(_packet([_row("1m", 105, 90, 120, True),
                                     _row("1y", 140, 80, 200, False)]))
