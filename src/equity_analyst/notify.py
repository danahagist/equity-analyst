...
def build_message(
    ...
    for path in attachments or []:
        ...
        message.add_attachment(data, maintype="text", subtype=subtype, filename=path.name)
    return message
...
def send_email(
    ...
    *,
    ...
    """Send the run email via SMTP. Raises if SMTP isn't configured.

    Port 465 uses implicit TLS (SMTP_SSL); anything else uses STARTTLS — plain
    SMTP + starttls() against a 465 socket hangs until timeout with a confusing
    disconnect error.
    """
